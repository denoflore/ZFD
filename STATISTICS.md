...
- **Gallows-initial position claim**: Original claim that gallows appear "disproportionately word-initial" showed 14% actual vs 19% random expectation when testing standalone gallows characters (k,t,f,p) at position 0. However, the test was badly designed. The Voynich operator system uses multi-character prefixes (qo-, ch-, sh-, da-, ok-, ot-), not standalone gallows. Re-analysis confirmed: 82.7% of tokens start with an identified operator prefix, operators reduce suffix entropy by 26.2% on average, and different operators select substantially different suffix repertoires (average Jaccard similarity 0.082). **Resolved: CONFIRMED.** The front-loading is in the operator system, not in individual gallows characters.
