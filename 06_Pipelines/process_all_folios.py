...
from zfd_decoder_v2 import ZFDDecoder, LEXICON_PATH


def lexicon_identity():
    """Computed morpheme count and version from the canonical lexicon.
    Never hardcode these: a stale count in every generated page was the
    exact defect the 2026-08-25 audit found."""
    with open(LEXICON_PATH) as handle:
        lexicon = json.load(handle)
    count = sum(len(lexicon[key]) for key in
                ("operators", "stems", "suffixes", "state_markers",
                 "latin_terms") if key in lexicon)
    version = lexicon.get("meta", {}).get("version", "unknown")
    return version, count


LEXICON_VERSION, MORPHEME_COUNT = lexicon_identity()

REPO_ROOT = Path(__file__).resolve().parent.parent
PIPELINE_DIR = REPO_ROOT / "08_Final_Proofs" / "Master_Key"
OUTPUT_DIR = REPO_ROOT / "translations" / "recipes"
EVA_DIR = REPO_ROOT / "voynich_data" / "raw_eva"
LEXICON_SOURCES = REPO_ROOT
...
def generate_recipe_markdown(folio_id, decoded, iiif_map):
    ...
    md.append(f"**Decoder:** ZFD v2.0 (unified lexicon v{LEXICON_VERSION}, {MORPHEME_COUNT} morphemes)")
    md.append(f"\n---\n")
    ...
    # Footer
    md.append("---\n")
    md.append(f"*Generated by ZFD Decoder v2.0 | Unified Lexicon v{LEXICON_VERSION} ({MORPHEME_COUNT} morphemes) | {folio_id}*")
...
def process_all_folios():
    ...
    summary = {
        ...
        "lexicon_version": f"{LEXICON_VERSION} ({MORPHEME_COUNT} morphemes)",
