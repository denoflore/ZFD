...
> Historical legacy record. The claims and numerical confidence statements in
> this document predate the image native rebuild. They are retained for audit
> and falsification. They are not current findings and cannot authorize a
> publication claim. The current authority is the repository root README and
> `data/image_native/claim_ledger.json`.

## Evidence status: image native rebuild in progress

The repository preserves the historical ZFD hypothesis and its generated
decoder, analysis, and translation artifacts. Those artifacts are under a new
pixel grounded audit. They are not accepted as primary OCR evidence.

As of 28 July 2026:

- OCR accuracy is not measured. An adjudicated, leakage resistant, image aligned
  gold set does not yet exist.
- Translation remains unresolved. No page is confirmed translated until its
  exact image hash, text region, OCR, diplomatic reading, terminology analysis,
  modern Croatian, English layers, reviewer, and adjudicator pass parity.
- Ragusan provenance remains an unproven hypothesis requiring direct period
  controls and independent historical review.
- Existing Voynich transcriptions and transliterations are quarantined from the
  primary image recognition lane.
- The current machine authority is
  [`data/image_native/claim_ledger.json`](data/image_native/claim_ledger.json).

See [the image native OCR contract](docs/IMAGE_NATIVE_OCR.md),
[evidence boundary](docs/EVIDENCE_BOUNDARY.md), and
[provenance status](docs/PROVENANCE_STATUS.md).

## Historical ZFD record

**Initial pre-release:** August 2025

**Historical scope:** Writing system mechanics, morphology, and constrained
functional semantics derived from inherited text records.

**Legacy decoder coverage:** 92.1% morphological token coverage under the frozen
lexicon. This is character coverage inside the legacy decoder and is not OCR or
semantic confidence.

The historical reports below remain available for falsification and comparison.
Their completion and provenance language is superseded by the evidence status
above until the image native gates allow it.
...
**Historical ZFD hypothesis: the Voynich Manuscript may be a Ragusan pharmaceutical recipe book.** The Franciscan Pharmacy was proposed as a candidate institution based on its medieval recipe tradition and commodity overlap. This attribution is currently blocked by the image native and comparative control requirements in the claim ledger.
...
| 7 | Audited legacy Interlinear Quadrilingual | Generated records exist across 201 folios; image to translation parity is unverified. |
