...
def test_chol():
    ...
    assert result.operator == "h", f"ch maps to h, got {result.operator}"
    assert "ol" in result.zfd, f"ol preserved, got {result.zfd}"
    print(f"✓ chol → {result.zfd} ({result.english})")
