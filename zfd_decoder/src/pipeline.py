...
class ZFDPipeline:
    def __init__(self, data_dir: str = "data", lexicon_file: str = None):
        ...
        # Prefer lexicon_v2, fall back to v1
        if lexicon_file:
    ...
    def process_token(self, token: Token) -> Token:
        ...
        # a known lexicon entry. This prevents operator stripping and gallows
        # expansion from destroying known words like 'dain' (given/added) -> da+in,
        # 'dar' (gift) -> da+r, 'fac' (face) -> plrac, etc.
        # Priority: exact whole-word match in lexicon beats any decomposition.
        precheck_word = token.eva
        precheck_data = self.lexicon.lookup(precheck_word)

        if precheck_data:
            # Known word -- skip operator stripping and gallows entirely
    ...
    def _generate_diagnostics(self, lines: List[List[Token]]) -> Dict:
        ...
        for t in all_tokens:
            if t.stem_known:
                cat = self.lexicon.get_category(t.stem)
                category_counts[cat] = category_counts.get(cat, 0) + 1
                stem_data = self.lexicon.lookup(t.stem)
                if stem_data and stem_data.get('source', '') == 'miscellany_new':
                    miscellany_count += 1
